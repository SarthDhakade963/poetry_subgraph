...
from app.graph.state import EmotionalVibe, GeneralAttributes, HumanPresence, PoemState, Scene, VideoState
...
async def context_node(state: PoemState):
    ...
    general = video_json.general_attributes or GeneralAttributes()

    scene = general.scene or Scene()
    human_presence = general.human_presence or HumanPresence()
    emotional_vibe = general.emotional_vibe or EmotionalVibe()
...
async def emotion_node(state: PoemState):
    video_json = state.video_json
    general = video_json.general_attributes or GeneralAttributes()
    ...
    energy_level = emotional_vibe.energy_level
    visible_emotions = human_presence.visible_emotions

    core_message = state.core_message
    moment_description = state.moment_description
    underlying_theme = state.underlying_theme
...
async def tone_node(state: PoemState):
    ...
    tone_result = await extract_tone(mood, narrative_voice)

    return {
        "selected_tone": tone_result.get("selected_tone")
    }
